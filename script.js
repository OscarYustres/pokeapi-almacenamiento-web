...
function crearTarjetas(informacionDePersonajes) {
    // Apunta de forma única a la grilla de abajo
    const containerTarjetas = document.getElementById('lista-general');
...
// Inicializa favoritos apenas cargan los datos principales
...
function crearTarjetaPersonajeEncontrado(listaPersonajes) {
    ...
    if (!containerResultadosBusqueda) return;
...
function guardarEnFavoritos(pokemon, boton) {
    ...
    if (!existe) {
        ...
        for (const fav of favoritosActuales) {
            if (fav.id !== pokemon.id) {
                nuevaLista.push(fav);
            }
        }

        favoritosActuales = nuevaLista;
        localStorage.setItem('pokemonFavoritos', JSON.stringify(favoritosActuales));
        boton.textContent = "🤍";
...
function actualizarSeccionFavoritos() {
    ...
    favoritosActuales.forEach(pokemonFav => {
        let tarjeta = document.createElement('div');
        tarjeta.innerHTML = `
        <div class="pokemon-tarjeta">
            <img src="${pokemonFav.image}" alt="image${pokemonFav.name}">
            <p class="nombre">${pokemonFav.name}</p>
        </div>`;
